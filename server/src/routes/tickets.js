...
function nextTicketNumber() {
  ...
  return `INC-${String(1000 + row.c + 1)}`;
}
...
router.get('/:id', requireAuth, (req, res) => {
  ...
  res.json({ ticket, comments, history, approvals, linkedAssets, csat: csat || null });
});
...
router.post('/', requireAuth, async (req, res) => {
  ...
  db.prepare(
    `INSERT INTO tickets (id, number, type, title, description, priority, category, subcategory, team, requester_id, sla_due_at, response_due_at, sla_policy_id, source, risk, planned_start, planned_end, rollback_plan, cab_status)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(
    id, number, type, title, description || '', priority, category || null, subcategory || null, team || null,
  ...
  if (isChange) {
    ...
    const cabMembers = db.prepare("SELECT id FROM users WHERE role = 'admin'").all();
    for (const m of cabMembers) notifyUser(m.id, 'CAB approval requested', `${number} — ${title}`, `/tickets/${id}`);
  }
...
router.patch('/:id', requireAuth, (req, res) => {
  ...
  if (req.body.status === 'resolved') {
    notifyUser(ticket.requester_id, 'Your ticket was resolved', `${ticket.number} — ${ticket.title}. Let us know how we did!`, `/tickets/${ticket.id}`);
  }
...
router.post('/:id/assets', requireAuth, (req, res) => {
...
router.post('/:id/ai/summarize', requireAuth, async (req, res) => {
  try {
    const ticket = db.prepare('SELECT * FROM tickets WHERE id = ?').get(req.params.id);
    if (!ticket) return res.status(404).json({ error: 'Not found' });
    const comments = db.prepare('SELECT * FROM ticket_comments WHERE ticket_id = ? ORDER BY created_at').all(ticket.id);
...
router.post('/:id/ai/suggest-resolution', requireAuth, async (req, res) => {
  try {
    const ticket = db.prepare('SELECT * FROM tickets WHERE id = ?').get(req.params.id);
    if (!ticket) return res.status(404).json({ error: 'Not found' });
    const comments = db.prepare('SELECT * FROM ticket_comments WHERE ticket_id = ? ORDER BY created_at').all(ticket.id);
...
router.post('/:id/ai/categorize', requireAuth, async (req, res) => {
  try {
    const ticket = db.prepare('SELECT * FROM tickets WHERE id = ?').get(req.params.id);
    if (!ticket) return res.status(404).json({ error: 'Not found' });
    const provider = getProvider(req.body.provider_id);
