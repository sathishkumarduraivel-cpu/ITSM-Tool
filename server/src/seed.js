...
function upsertUser(name, email, password, role, team) {
  ...
  db.prepare('INSERT INTO users (id, name, email, password_hash, role, team, avatar_color) VALUES (?,?,?,?,?,?,?)').run(
    id, name, email, bcrypt.hashSync(password, 10), role, team || null, colors[Math.floor(Math.random() * colors.length)]
  );
  return id;
}
...
if (ticketCount === 0) {
  ...
  for (const s of samples) {
    ...
    db.prepare(
      `INSERT INTO tickets (id, number, type, title, description, status, priority, category, team, requester_id, assignee_id, sla_due_at, created_at, updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`
    ).run(id, number, s.type, s.title, s.description, s.status, s.priority, s.category, s.team, s.requester_id, s.assignee_id, sla_due_at, created_at, created_at);
...
if (assetCount === 0) {
  ...
  for (const a of assets) {
    db.prepare(
      'INSERT INTO assets (id, tag, name, type, status, owner_id, vendor, location) VALUES (?,?,?,?,?,?,?,?)'
    ).run(uid('ast'), a.tag, a.name, a.type, a.status, a.owner_id || null, a.vendor || null, a.location || null);
  }
...
if (kbCount === 0) {
  ...
  for (const a of articles) {
    db.prepare('INSERT INTO kb_articles (id, title, category, body, tags, author_id) VALUES (?,?,?,?,?,?)').run(
      uid('kb'), a.title, a.category, a.body, a.tags, adminId
    );
  }
  console.log(`Seeded ${articles.length} KB articles.`);
}

const autoCount = db.prepare('SELECT COUNT(*) c FROM automations').get().c;
if (autoCount === 0) {
  const workflows = [
    {
  ...
  for (const w of workflows) {
    db.prepare('INSERT INTO automations (id, name, description, enabled, trigger, conditions, actions) VALUES (?,?,?,?,?,?,?)').run(
      uid('wf'), w.name, w.description, 1, JSON.stringify(w.trigger), JSON.stringify(w.conditions), JSON.stringify(w.actions)
    );
  }
...
// ---- Service Catalog ----
const catCount = db.prepare('SELECT COUNT(*) c FROM catalog_categories').get().c;
if (catCount === 0) {
  const hwCat = uid('cat');
  const swCat = uid('cat');
  const accCat = uid('cat');
  db.prepare('INSERT INTO catalog_categories (id, name, icon, sort_order) VALUES (?,?,?,?)').run(hwCat, 'Hardware', 'Laptop', 1);
  db.prepare('INSERT INTO catalog_categories (id, name, icon, sort_order) VALUES (?,?,?,?)').run(swCat, 'Software', 'AppWindow', 2);
  db.prepare('INSERT INTO catalog_categories (id, name, icon, sort_order) VALUES (?,?,?,?)').run(accCat, 'Access', 'KeyRound', 3);
  ...
  for (const it of items) {
    db.prepare(
      `INSERT INTO catalog_items (id, category_id, name, description, form_schema, approval_required, approver_role, default_priority)
       VALUES (?,?,?,?,?,?,?,?)`
    ).run(uid('itm'), it.category_id, it.name, it.description, JSON.stringify(it.form_schema), it.approval_required, it.approver_role, it.default_priority);
  }
  console.log(`Seeded 3 catalog categories and ${items.length} catalog items.`);
}

// ---- SLA policies ----
const slaCount = db.prepare('SELECT COUNT(*) c FROM sla_policies').get().c;
if (slaCount === 0) {
  const policies = [
  ...
  for (const p of policies) {
    db.prepare('INSERT INTO sla_policies (id, name, priority, response_minutes, resolution_minutes) VALUES (?,?,?,?,?)').run(
      uid('sla'), p.name, p.priority, p.response_minutes, p.resolution_minutes
    );
  }
  db.prepare('DELETE FROM business_hours').run();
  for (let d = 1; d <= 5; d += 1) {
    db.prepare('INSERT INTO business_hours (id, day_of_week, start_time, end_time) VALUES (?,?,?,?)').run(uid('bh'), d, '09:00', '17:00');
  }
  console.log(`Seeded ${policies.length} SLA policies and Mon–Fri 9-5 business hours.`);
}

// ---- Contracts & Purchase Orders ----
const contractCount = db.prepare('SELECT COUNT(*) c FROM contracts').get().c;
if (contractCount === 0) {
  db.prepare('INSERT INTO contracts (id, vendor, name, type, start_date, end_date, value, renewal_notice_days) VALUES (?,?,?,?,?,?,?,?)').run(
    uid('ctr'), 'Microsoft', 'Microsoft 365 E3 (50 seats)', 'license', '2025-09-01', '2026-08-31', 24000, 45
  );
  db.prepare('INSERT INTO contracts (id, vendor, name, type, start_date, end_date, value, renewal_notice_days) VALUES (?,?,?,?,?,?,?,?)').run(
    uid('ctr'), 'Dell', 'Dell ProSupport Plus', 'support', '2025-01-01', '2026-12-31', 8000, 30
  );
  db.prepare('INSERT INTO purchase_orders (id, po_number, vendor, item, amount, status, ordered_date) VALUES (?,?,?,?,?,?,?)').run(
    uid('po'), 'PO-1001', 'Lenovo', '10x ThinkPad X1 Carbon', 18500, 'ordered', '2026-07-20'
  );
...
if (tplCount === 0) {
  ...
  for (const t of templates) {
    db.prepare('INSERT INTO notification_templates (id, event, channel, subject, body) VALUES (?,?,?,?,?)').run(uid('tpl'), t.event, t.channel, t.subject, t.body);
  }
