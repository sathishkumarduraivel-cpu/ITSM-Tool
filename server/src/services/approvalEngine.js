...
export function resolveTicketAfterApprovalChange(ticketId) {
  ...
  const tpl = renderTemplate('change_approved', { number: ticket.number, title: ticket.title });
  notifyUser(ticket.requester_id, tpl?.subject || 'Request approved', tpl?.body || `${ticket.number} — ${ticket.title} was approved.`, `/tickets/${ticket.id}`);
}
...
export function autoApprovePending(ticketId) {
  ...
  if (pending.length) resolveTicketAfterApprovalChange(ticketId);
