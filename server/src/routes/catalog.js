...
router.post('/categories', requireAuth, requireRole('admin'), (req, res) => {
  ...
  const id = uid('cat');
  db.prepare('INSERT INTO catalog_categories (id, name, icon, sort_order) VALUES (?,?,?,?)').run(id, name, icon, sort_order);
...
router.post('/items', requireAuth, requireRole('admin'), (req, res) => {
  ...
  const id = uid('itm');
  db.prepare(
    `INSERT INTO catalog_items (id, category_id, name, description, icon, form_schema, approval_required, approver_role, default_priority, price)
     VALUES (?,?,?,?,?,?,?,?,?,?)`
  ).run(id, category_id || null, name, description || '', icon, JSON.stringify(form_schema), approval_required ? 1 : 0, approver_role, default_priority, price || null);
  res.status(201).json({ id });
});

router.patch('/items/:id', requireAuth, requireRole('admin'), (req, res) => {
...
router.post('/items/:id/request', requireAuth, (req, res) => {
  ...
  db.prepare(
    `INSERT INTO tickets (id, number, type, title, description, status, priority, category, requester_id, sla_due_at, source, catalog_item_id, catalog_form_data)
     VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`
  ).run(
    id, number, 'request', item.name,
  ...
  if (needsApproval) {
    ...
    // notify anyone with the approver role — simplified to all admins for this MVP
    const approvers = db.prepare('SELECT id FROM users WHERE role = ?').all(item.approver_role);
    for (const a of approvers) {
      notifyUser(a.id, 'Approval requested', `${req.user.name} requested "${item.name}" (${number})`, `/tickets/${id}`);
    }
  }
