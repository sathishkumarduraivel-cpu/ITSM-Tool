...
export default function NotificationBell() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen((o) => !o)} className="relative text-slate-500 hover:text-slate-800 p-1.5 rounded-lg hover:bg-slate-100">
        <Bell size={18} />
        {unread > 0 && (
        ...
        <div className="absolute right-0 mt-2 w-80 card shadow-lg z-50 max-h-96 overflow-y-auto">
          ...
          {items.map((n) => (
            <button
              key={n.id}
              onClick={() => openItem(n)}
              className={`w-full text-left px-3 py-2.5 border-b border-slate-50 hover:bg-slate-50 ${!n.read ? 'bg-brand-50/40' : ''}`}
            >
